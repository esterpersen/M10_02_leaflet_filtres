...
function render_to_map(array, filter) {
	...
	if (filter === "all" || filter) {
		$.each(array, function (key, item) {
			...
			if (filter === "all") {
				displayEachMarker(latEl, lngEl, nomRestaurant, kfs, direccio);
...
function displayEachMarker(lat, lng, nameRest, kfs, direccio) {
	let marker = L.marker([lat, lng])
		.bindPopup("<b><h5 class='restName'>" + nameRest.toUpperCase() + "</h5></b> <br><b> Direcció: </b>" + direccio + " <br><b>Lat: </b>" + lat.toString() + "<b> Lng: </b>" + lng.toString() + "<br><b>Tipus de menjar que hi trobaras son: </b>" + kfs);
